...
- `js/main.js` assembles the page; `js/data.js` is the static content entry point; `js/components/` contains section renderers. Automatically curated Thoughts live in `js/thoughts-data.js` and are re-exported from `js/data.js`, so the profile editor cannot overwrite synchronized notes.
