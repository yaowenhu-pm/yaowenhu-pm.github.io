...
| 网址 | 内容 |
|---|---|
| `/` | 自我介绍、头像与联系方式、教育经历、实习经历 |
| `/work/` | 作品、在别处找我 |
| `/thoughts/` | 随想 |

原来的 `/#work`、`/#thoughts`、`/#elsewhere`、`/#note-…` 会转到对应新页面，保留单篇定位。经历首页保留原有的个人简介、头像、邮件/微信联系方式和教育经历，置于实习经历之前。
...
- 随想页普通访问直接使用预渲染正文，JS 只绑定交互，避免缓存中的旧数据覆盖新页面。经历、作品页保留从数据渲染的能力，兼容现有网页编辑器的资料更新。构建为 JS 依赖生成内容版本映射，并按 `styles/main.css` 的顺序直接引用带版本的样式文件；资源更新不依赖强制刷新。

## 更新约定

- 个人资料改动在 `js/data.js`，随想在 `js/thoughts-data.js`，样式改动遵循 `tokens.css` 的变量体系。
- “随想”每条仅有 `id`、原始发表日期 `date`、阅读标题 `title`、原文 `content`。按日期倒序，默认展开最新一篇，其余点击展开；全部正文也会预渲染，单条可用 `#note-…` 定位。展示时去掉段落间空行，保留正常换行及文字中的空格，源数据仍保留原文。
...
- 内容或组件改完必须跑 `node build.js`，使浏览器和爬虫读到更新后的正文与资源版本。资源版本不包含随想数据，纯随想同步仍只改随想数据文件与该页预渲染区块。
- 简历 PDF 与实际投递版本保持一致。
